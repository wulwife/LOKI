...
class Loki:
    ...
    def coherence_plot(self, event_path, corrmatrix, xax, yax, zax, itrial, normalization=False):
        ...
        cs = plt.contourf(xax, yax, CXY, 20, cmap=cmap, interpolation='linear', norm=nrm)
        ax.set_xlabel('X (km)')
        ax.set_ylabel('Y (km)')
        ...
        cs = plt.contourf(xax, zax, CXZ, 20, cmap=cmap, interpolation='linear', norm=nrm)
        ax.set_xlabel('X (km)')
        ax.set_ylabel('Z (km)')
        cbar = plt.colorbar(cs)
        ax.invert_yaxis()
        ...
        ax = fig.gca()
        cmap = plt.cm.get_cmap('jet', 100)
        cs = plt.contourf(yax, zax, CYZ, 20, cmap=cmap, interpolation='linear', norm=nrm)
        ax.set_xlabel('Y (km)')
        ax.set_ylabel('Z (km)')
        cbar = plt.colorbar(cs)
        ax.invert_yaxis()
